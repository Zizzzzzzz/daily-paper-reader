...
tldr: 现有Video LLM处理长视频时，因视觉token预算有限且难以捕获多个关键事件，通常采用两阶段方法（选关键帧+详细感知），但缺乏自适应能力分配与自纠正机制，导致建模不可靠。为此，提出MoD-VLLM框架，通过正负视频片段定位模块与模块化动态粒度反射模块形成闭环，迭代地自我反思并逐步定位问题相关片段；同时引入动态粒度强化学习策略联合优化定位策略与视觉表示。在多个长视频基准及新提出的MEventBench上，MoD-VLLM显著超越现有基线。
source: arxiv
...
motivation: 长视频理解中现有方法缺乏自适应容量分配与自纠正机制，导致多事件建模不可靠。
method: 提出MoD-VLLM，包含正负视频片段定位模块和模块化动态粒度反射模块，形成闭环迭代定位；并引入动态粒度强化学习联合优化。
result: 在多个长视频理解基准和MEventBench上，MoD-VLLM显著超越当前最优基线。
conclusion: MoD-VLLM通过动态粒度模块化设计及强化学习，有效提升了多事件长视频理解的性能。
---

## 摘要
视频大语言模型在各种视频理解任务中取得了显著进展。然而，长视频场景仍然具有挑战性，因为有限的视觉标记预算与捕捉多个关键事件的需求之间存在张力。现有方法通常分两个阶段处理长视频，即i)选择关键帧和ii)进行详细感知，这存在局限性：它们缺乏自适应容量分配和自校正的模块化机制，导致建模不可靠。为了解决这些挑战，我们提出了MoD-VLLM，一种新颖的模块化动态粒度视频大模型框架，用于多事件长视频理解，它迭代地、自反思地统一了时间定位和语义理解。具体来说，我们提出了正负视频片段定位模块和模块化动态粒度反思模块，它们形成一个闭环，逐步定位与问题相关的视频片段。定位模块指导视频大模型根据视频问题区分相关和不相关的视频片段。反思模块采用模块化调度器，动态选择对相关正面片段进行细粒度编码以捕捉详细感知，对负面片段进行粗粒度编码以保持全局上下文。我们进一步提出了动态粒度强化学习策略，使MoD-VLLM能够联合学习最优定位策略和动态粒度视觉表示。此外，我们提出了MEventBench，一个具有挑战性的多事件长视频基准，用于复杂的长期视频推理。在多个长视频理解基准和我们的MEventBench上的大量实验表明，MoD-VLLM显著优于最先进的基线方法。
